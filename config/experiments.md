...
| contrastive.weight | 0.03→**0.01** |
| domain.weight | 0.05→**0.03** |

### 图像修复细节（`dataset_77sets.py`）

原：`get_window(img, 40/80, 80/200, 40/380)` — CT 窗，MRI 值域 0-1000，Ch1 信号被截断 50%+

新：`zscore_window(img, -3~3, -1.5~1.5, -5~5)` — 每张图的均值±标准差，保留全动态范围

### 配置总览

| 参数 | V5 | V6 |
|------|----|-----|
| MRI 窗 | CT HU 窗 | **z-score (MRI 适配)** |
| 77sets 增强 | 基础 | **+RandBiasField** |
| batch_size | 4 | **6** (加速30%) |
| contrastive.weight | 0 (off) | **0.01** |
| domain.weight | 0.05 | **0.03** |
| n_slots/n_prototypes | 8/8 | — |
| focal.alpha | 0.85 | — |
| T_0 / accum | 15 / 8 | — |

**预期:** MRI 信号修复 + 极轻量对比损失协同作用。77sets AUC 0.82~0.85, ISLE 0.78~0.82。如果能达到，两数据集 AUC 均超 Tri-CAF。
